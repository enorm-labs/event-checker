-- Lido credits "Atoc Live", which the descriptor strip stored as `atoc` / "Atoc"; the company is
-- ATOC Soundlab, and the normalizer now pins that name (#1343). This moves the row that exists onto
-- the slug the normalizer resolves to. The same four-step shape as V029, keyed on slug and a no-op
-- where a row is absent.
--
...
INSERT INTO promoter_merge (loser, survivor, survivor_name) VALUES
    ('atoc',      'atoc-soundlab', 'ATOC Soundlab'),
    ('atoc-live', 'atoc-soundlab', 'ATOC Soundlab');
