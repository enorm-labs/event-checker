...
/**
 * Unit tests for [PeterEdelWebsiteImporter].
 *
 * Uses a static HTML fixture and a mocked [HtmlFetcher] for deterministic, offline-safe testing.
 * Kulturhaus Peter Edel is a single-page source, so the importer's job is limited to the fetch →
 * scrape → `ImportResult` path: no detail pages are requested.
 */
class PeterEdelWebsiteImporterTest {
    private val htmlFetcher: HtmlFetcher = mockk()
    private val importer = PeterEdelWebsiteImporter(htmlFetcher)
    private val listingUrl = "https://www.peteredel.de/events/"
    ...
    fun `eventSource matches expected enum value`() {
        importer.eventSource shouldBe EventSource.PETER_EDEL
    }
    ...
    @Test
    fun `imports the whole programme and propagates conditional headers`() =
        runTest {
            stubListing()
