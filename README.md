...
1. Implement "Hello world" to verify that the setup works
    ...
    * Setup vue app in events-frontend and make an HTTP GET request to the backend endpoint when the page loads. Display the response ("Hello world") on the page.
...
6. Create Template Repository from this project in my Enterprise Repository and my private Repository on GitHub.
   * with `.github` directory and workflows, instructions, skills, prompts,and agents
   * with README, CONTRIBUTING, etc. (see GitHub docs and best practices)
...
* Focus on all music events (concerts, club nights, festivals etc. - not only Techno/Electronic music) in venues (clubs, convert halls etc.) of Berlin (maybe later also other cities)
...
This will also start the services (database) defined in the [compose.yaml](./compose.yaml) file via Spring's [Docker Compose Support](https://docs.spring.io/spring-boot/reference/features/dev-services.html#features.dev-services.docker-compose).
