...
request it opens starts no check, so the bump can never merge. An App is a different identity, so both fire.

It has **no expiry**, so it takes no row in `credential-expiry-reminder.yml`. The private key is shown once at
download and is not recoverable. Generating a new one in the App settings replaces it, so a lost key costs one
`gh secret set` rather than a new App.
...
| 21  | **healthchecks.io ping URLs** ⚠️      | One opaque UUID per environment. **Anyone holding one can suppress the alarm**, and the failure it causes is _silence_ | `/etc/wal-g/credentials.env` on the node, the **`HEALTHCHECKS_PING_URL` Actions secret** for `site-production`, and the dashboard | No — readable from the dashboard              |
