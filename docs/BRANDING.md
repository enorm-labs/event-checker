...
### 5.12 What the de-slop run changed, and where the rules now live

Thirteen issues in September 2026 checked this site against eleven published signs that a page was generated by AI. Twelve were in scope. What changed:

| Change                                                            | Issue |
| ----------------------------------------------------------------- | ----- |
| `tw-animate-css` deleted, unused since the shadcn scaffold        | #1238 |
| The eyebrow label spent once per page, not above every title      | #1239 |
| The filter bar became chrome: one rule, no border, fill or radius | #1240 |
| The card lost its resting shadow and its hover lift               | #1241 |
| The home hero halved its padding and kept its centring            | #1243 |
| Six named type steps, §5.8                                        | #1244 |
| A 512 px derivative, and 3:2 measured against the corpus, §5.4    | #1245 |
| The cards turned inside out: poster first, no chrome at all       | #1246 |
| The pills flattened into a meta line, §5.11                       | #1248 |
| The flows walked on staging, which found four dead ends           | #1249 |

**Two were built and then rejected**, which is worth as much as the ten that shipped. The pulsing live dot stays: it marks the most time-sensitive fact on a
card, it is the only animation on the site, and `motion-safe` already silences it (#1242). The double-width lead tile was built, screenshotted against real
data and dropped, because the first event of a day is whatever starts earliest, so the big tile landed on an arbitrary event (#1247). A curated version is
#1262.

**The rules an agent has to obey are not in this file.** They are in
[`.github/instructions/design.instructions.md`](../.github/instructions/design.instructions.md), which loads itself whenever `events-frontend/src/` is
touched, and [docs/DESIGN.md](DESIGN.md) explains that split. This file keeps the argument, because prose is not a constraint: where a vibe and a rule
conflict, a model sides with the vibe.
