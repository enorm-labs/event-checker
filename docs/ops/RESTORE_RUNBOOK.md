...
**Wait for the promotion before you judge it.** `pg_ctl -w start` returns as soon as the server accepts connections, and a cluster still in recovery accepts
them. Replay to the target continues after that, and every WAL segment on the way is a separate fetch from the bucket. A `pg_is_in_recovery()` asked
immediately answers `t` on a perfectly healthy restore:

```sh
until [ "$(sudo -u postgres psql -tAX -h /tmp -p 5433 -c 'select pg_is_in_recovery()')" = f ]; do sleep 1; done
```

Then confirm it stopped where you meant:

```sh
sudo grep -E 'recovery stopping|last completed transaction' /tmp/drill.log
# recovery stopping before commit of transaction 85744, time 2026-09-09 11:42:59.104105+00
# last completed transaction was at log time 2026-09-09 11:42:42.951538+00
```
...
correct behaviour. **If the cluster is still in recovery** after the wait above, `recovery_target_action = 'promote'` did not fire. The target is usually
beyond the end of the archived WAL.
...
until ! pgrep -f '[p]ostgres -D /var/lib/postgresql/drill' >/dev/null && ! pgrep -f '[w]al-g wal-fetch' >/dev/null; do sleep 1; done
sudo rm -rf /var/lib/postgresql/drill /tmp/drill.log
```

**The `until` line is not decoration.** `pg_ctl -w stop` returns when the PID file is gone, and a writer can outlive that by a moment. The postmaster is one.
The other is `wal-g wal-fetch`, which `restore_command` starts as a child and which is still putting segments into `pg_wal`. A `rm -rf` that races either one
fails as `Directory not empty` on a directory the operator can see is nearly empty. Wait for both, and note the bracket: it keeps the pattern from matching the
command line of whatever runs it, which matters over `ssh host '…'`.
...
| **`pg_is_in_recovery()` is `t` right after the start**       | Not a failure yet. `pg_ctl -w start` returns when the server accepts connections, which a recovering cluster does. Wait for the promotion, §5                                                                                                                                                                                                                                                                            |
| **`rm -rf` on the scratch directory: `Directory not empty`** | The postmaster, or the `wal-g wal-fetch` that `restore_command` started, outlived `pg_ctl -w stop` and is still writing `pg_wal`. Wait for the processes rather than the PID file, §7                                                                                                                                                                                                                                    |
| **`pgrep -f` or `pkill -f` on the data directory path**      | The pattern matches the command line running it, so over `ssh host '…'` a `pkill` kills its own shell before the cleanup runs. Write it as `[p]ostgres -D …`                                                                                                                                                                                                                                                             |
