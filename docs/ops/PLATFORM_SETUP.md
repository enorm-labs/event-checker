...
flowchart LR
    ...
    tr["Traefik :443<br/>TLS terminates here<br/>concurrency limit + security headers"]
    dns --> tr
...
| 9   | **Traefik security headers + rate limiting**                             | A content security policy, HSTS, `X-Content-Type-Options`, frame options — below. The rate limit bounds concurrency and per-source request rate, both on since [#268](https://github.com/enorm-labs/event-junkie/issues/268) |
...
**Item 9's limit is two limits, and they bound different things.** `inFlightReq` bounds concurrent requests, grouped by request host, so it applies whatever
address reaches the edge — and it stops nothing that arrives sequentially. A scraper making 500 requests a second one at a time never trips it. `rateLimit`
groups by the client's address and is the half that sees that.

**The per-source half was disabled for a milestone, and #1013 is what turned it on.** ServiceLB was how k3s exposed Traefik. Its `klipper-lb` container
masqueraded every packet before Traefik read it. Traefik's peer was therefore the `svclb` pod for every visitor. The documented fix,
`externalTrafficPolicy: Local`, was not enough. The controller aimed klipper-lb at the node, and klipper-lb masqueraded anyway. What worked is a **hostPort**
on Traefik with the Service at `ClusterIP`, which takes ServiceLB out of the path. See §5, and each cluster's `traefik-host-ports.yaml`.

**Verified on production without turning a log on.** The node's `POSTROUTING` chain carries no `MASQUERADE` of its own. The one masquerade in the host-port
path is `CNI-HOSTPORT-MASQ`. It fires only on the `0x2000` mark, which the portmap plugin sets for localhost and pod-CIDR sources. Hairpin traffic is
rewritten, and an external visitor's address is not. Staging measured the same conclusion directly, from Traefik's access log, for one request.

**The numbers are large because one Ingress carries the whole site.** A page view spends the budget on its assets, fonts and venue images as well as its
queries. The chart's README carries the arithmetic. It also carries the escalation path, if the limit ever has to be tighter than the static surface allows.
...
installer the node booted with. [K3S_UPGRADE.md](K3S_UPGRADE.md) is the cheap path, and it is the one to take unless a rebuild is happening anyway.
