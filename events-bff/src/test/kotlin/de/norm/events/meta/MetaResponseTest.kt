...
class MetaResponseTest {
    ...
    fun `reports the stamped version and derives the short commit from the full sha`() {
        ...
        assertEquals("9f1a2b3c4d5e6f708192a3b4c5d6e7f809a1b2c3", response.commit)
        assertEquals("9f1a2b3", response.commitShort)
    }
    ...
    fun `reports the build time when the build stamped one`() {
        ...
        assertEquals("2026-08-07T12:49:19Z", response.buildTime)
    }

    @Test
    fun `omits the build time when the build stamped none`() {
        assertNull(MetaResponse.from(buildProperties("version" to "0.1.0")).buildTime)
    }
    ...
    fun `falls back to dev when the build stamped nothing`() {
        ...
        assertEquals(MetaResponse.DEV_VERSION, response.version)
        assertNull(response.commit)
        assertNull(response.commitShort)
    }
